test_that("CI_smd_ind_contrast compare to publshed examples", {
  ...
  testthat::expect_equal(round(res$effect_size, 4), sres[1, 1], tolerance = 1e-5)
  testthat::expect_equal(round(res$LL, 4), sres[1, 4], tolerance = 1e-5)
  testthat::expect_equal(round(res$UL, 4), sres[1, 5], tolerance = 1e-5)
  ...
  testthat::expect_equal(round(res$effect_size, 4), sres[1, 2], tolerance = 1e-5)
  testthat::expect_equal(round(res$LL, 4), sres[1, 4], tolerance = 1e-5)
  testthat::expect_equal(round(res$UL, 4), sres[1, 5], tolerance = 1e-5)
